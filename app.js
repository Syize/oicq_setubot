...
function DownloadPicPublic(path, image_name, callback, user_id) {
    process.exec("proxychains4 -q wget " + path + " -P /home/syize/Downloads/setu", function (error, stdout, stderr) {
        if (error !== null) {
            callback.sendGroupMsg(user_id, '获取图片失败');
        }
        else {
            callback.sendGroupMsg(user_id, oicq_1.segment.image("/home/syize/Downloads/setu/" + image_name, true, 15));
        }
    });
}
// 真的垃圾，完全跑不动
                    ...
                    res.on('end', function () {
                        ...
                        try {
                            if (parseData.error == '') {
                                ...
                                // http.request(http_proxy_option, (res)=>{
                                //     res.setTimeout(60, ()=>{
                                //         console.log('Get Pic Timeout')
                                //     })
                                ...
                                //     })
                                //     res.on('end', ()=>{
                                //         writeFile("data/image/" + parseData.data[0].pid + "." + parseData.data[0].ext, image, "binary")
                                //         bot.sendPrivateMsg(data.user_id, msg_text)
                                //         bot.sendPrivateMsg(data.user_id, "[CQ:flash,file=data/image/" + parseData.data[0].pid + "." + parseData.data[0].ext + "]")
                                //     })
                                // })
                            }
                            else {
